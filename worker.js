...
export default {
  async fetch(request, env) {
    try {
      ...
      if (!env.MEETINGS_KV) {
        return htmlResponse(renderSetupError());
      }
      ...
      if ((path === "/api-dock.php" || path === "/api-dock") && request.method === "POST") {
        return jsonResponse(await handleDockPost(request, env));
      }

      if ((path === "/api-dock.php" || path === "/api-dock") && request.method === "GET") {
        return jsonResponse(await getDockState(env));
      }

      if ((path === "/api-meeting.php" || path === "/api-meeting") && request.method === "GET") {
      ...
      if (request.method === "GET") {
        ...
        return htmlResponse(renderApp(view));
      }
...
function renderApp(initialView) {
  return `<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
...
<header class="topbar">
  ...
  <div class="room-state" id="roomState"><strong id="liveClock">--:--</strong></div>
</header>
...
<script>window.APP_INITIAL_VIEW=${JSON.stringify(initialView)};</script>
<script>
